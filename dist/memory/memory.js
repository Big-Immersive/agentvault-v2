...
export function loadMemories(projectDir) {
    const { memory: memPath } = resolvePaths(projectDir);
    const passphrase = getPassphrase(projectDir);
    return readEncryptedFile(memPath, passphrase, []);
}
/** Save all memory entries (encrypted) */
export function saveMemories(projectDir, entries) {
...
export async function storeMemory(projectDir, opts) {
...
export async function queryMemories(projectDir, query, limit = 10) {
    return memoryMutex.runExclusive(() => {
        const { memory: memPath, base } = resolvePaths(projectDir);
        ensureDir(projectDir);
        const release = lockfile.lockSync(base, { lockfilePath: memPath + '.lock' });
...
export async function listMemories(projectDir, opts) {
    return memoryMutex.runExclusive(() => {
        const entries = loadMemories(projectDir);
...
export async function removeMemory(projectDir, key) {
    return memoryMutex.runExclusive(() => {
        ensureDir(projectDir);
        const { memory: memPath, base } = resolvePaths(projectDir);
        const release = lockfile.lockSync(base, { lockfilePath: memPath + '.lock' });
...
export async function exportMemories(projectDir) {
    return memoryMutex.runExclusive(() => {
