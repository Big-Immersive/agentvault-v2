...
export function secretCommand(): Command {
  ...
  cmd.command('get <key>')
    ...
    .action((key: string) => {
      ...
      if (value === undefined) { console.log(`Secret "${key}" not found.`); return; }
      console.log(value);
    });
  ...
  cmd.command('remove <key>')
    ...
    .action((key: string, opts) => {
      ...
      if (removed) console.log(`Secret "${key}" removed`);
  ...
  cmd.command('list')
    ...
    .action(() => {
      ...
      if (!keys.length) { console.log('No secrets stored.'); return; }
