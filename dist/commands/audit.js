...
export function auditCommand() {
    ...
    cmd.command('show')
        ...
        for (const e of entries) {
            ...
            console.log(`${time} ${e.sessionId.slice(0, 8).padEnd(10)} ${e.agentId.padEnd(16)} ` +
    ...
    cmd.command('export')
        ...
        if (!entries.length) {
            console.log('No audit entries to export.');
            return;
        }
        let content;
        if (opts.format === 'csv') {
            const header = 'id,sessionId,agentId,profileName,varName,action,timestamp';
            const rows = entries.map(e => `${e.id},${e.sessionId},${e.agentId},${e.profileName},${e.varName},${e.action},${e.timestamp}`);
